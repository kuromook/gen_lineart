...
def features(pts, wmed, wstd, wfill, ctx_dist, ctx_dirs, ctx_cent, ctx_mask):
    path = order_path(pts)
...
def main():
    ...
    for i, (n, (g, sk, cs)) in enumerate(tiles.items()):
        ...
        for ti, c in enumerate(c for c in cs if len(c) >= a.min_token_px):
            ...
            for kind, cand in cands.items():
                ...
                f = features(pts, wmed, wstd, wfill, cd, dirs, cent, ctx)
    ...
    cols = ["tile", "token", "kind", "label", "on_other"] + [k for k in rows[0] if k[:2] in ("a_", "b_")]
