...
def materialize_rough(rough_gray, cx, cy, out_w, out_h, dx, dy, scale, pad=170):
    # pad must exceed |dx|/|dy| or the y0/x0 crop below goes negative or past
    # the ROI edge (silent wraparound / wrong crop, not a clean crash). The
    # coarse-to-fine search added to match_koma_panels.py's
    # search_panel_alignment() can now return offsets well beyond the old
    # fixed default, so size pad to the actual offset instead of assuming it.
...
def main():
    ...
    with zipfile.ZipFile(args.zip_path) as zf:
        ...
        # Single-entry cache: `accepted` rows are page-grouped (generated by
        # match_koma_panels.py processing pages in order), so this only
        # reloads on an actual page change. A never-cleared dict here would
