...
### 解像度スイープ完走 — trackの前提が2つ覆った (2026-09-06 08:12〜09:53)

`experiments/run_resolution_sweep_20260906.sh`。4モデル(anime/manga ×
LoRAあり/なし) × 512/768/1024 × cs 0.5/1.0/2.0 = 36セル + オフロード対照2。
結果 `results/resolution_sweep_20260906/`(`scores.csv`、`montage_best.png`、
`montage_by_res.png`)。

**土台の検証**: 512/cs1.0のオフロード無し2セルが11モデル表の
`0.0945`/`0.1226`を**delta 0.0000で再現**。さらに`--cpu-offload`の有無でも
出力が完全一致(delta 0.0000)。以降の数値は信頼できる。

#### 覆った前提1: 「SDXLはcsを上げると悪化する」は誤り

これは`anime_lora`という**1チェックポイント固有**の性質だった。1024でのcs応答:

| model | cs0.5 | cs1.0 | cs2.0 |
|---|---:|---:|---:|
| anime_base | 0.1686 | 0.2341 | **0.2568** ↑ |
| manga_base | 0.1541 | **0.2121** | 0.1910 |
| manga_lora | 0.1343 | 0.1751 | **0.1949** ↑ |
| anime_lora | 0.1217 | 0.1221 | 0.0432 ↓ |

4構成中3つでcsレバーは効く。SD1.5路線で見つけたレバーはSDXLでも生きていた。
briefingの「csレバーが効かない=別種の失敗」という切り分けは、
壊れたチェックポイント1本を見ての一般化だった。

#### 覆った前提2: グレー背景は`lineart_anime` ControlNet固有

ユーザーが目視で「anime側に一貫してグラデーションがかったグレーが乗る。
これはanimeを指定する限りついてくるようにみえる」と指摘。数値化のため
`bg_mode`/`near_white_frac`/`midtone_frac`をスコアラーに恒久追加
(`experiments/score_resolution_sweep_20260906.py`)。

スイープのラベルは`anime_*`=lineart_anime+coarse、`manga_*`=manga_line+
manga_lineと**2変数を同時に動かしていた**ため帰属が決まらない。
`experiments/run_grey_source_cross_20260906.sh`でLoRAなしの2×2
(ControlNet × 条件画像)を512/1024・cs1.0で実施。

`near_white_frac`(GT 0.948):

| | condCoarse | condManga |
|---|---:|---:|
| res512 cnAnime | 0.031 | 0.010 |
| res512 cnManga | 0.735 | 0.765 |
| res1024 cnAnime | 0.030 | 0.030 |
| res1024 cnManga | 0.914 | 0.878 |

**行が揃い列で差が出ない = ControlNet帰属**。ユーザーの仮説が正しかった。
`lineart_anime`は条件画像を替えても、解像度を512→1024にしても
(0.031→0.030)灰色のまま。

**当初「グレーは512とcs=1.0の産物」と書いたのは誤りだったので訂正する。**
正しくは1024とcs2.0が**揃ったときだけ**外れる交互作用:

`anime_base`の`near_white_frac`:

| | cs0.5 | cs1.0 | cs2.0 |
|---|---:|---:|---:|
| res512 | 0.250 | 0.031 | 0.118 |
| res768 | 0.165 | 0.022 | 0.625 |
| res1024 | 0.113 | 0.030 | **0.815** |

cs1.0では解像度をいくら上げても灰色、cs2.0でも512なら灰色。

#### ただし`manga_line`の白は「空白のページ」だった

`montage_grey_source.png`(剣タイル、cs1.0)の目視で、数値だけでは
見えないことが判明した。**`cnAnime`の2行は条件画像によらず剣を極めて
正確に描いている**(本調査中で最高の構造忠実度、ただし地はグレー)。
一方**`cnManga`の2行は512でも1024でも縞状・ブロック状に破綻していて
剣が存在しない**。

つまり`manga_base`の`near_white` 0.878〜0.914は「きれいな白い紙」では
なく「ほぼ空白のページ」。**上の落とし穴2で書いた「manga_baseは
インクが足りない」という表現も弱すぎた——描けていない**。

結論: **使うべきControlNetは`lineart_anime`**。グレーはその既知のコストで、
1024×cs2.0で外せる。`manga_line`は紙が白いだけで絵が出ていない。

なお2×2で、前trackが試していない交差組み合わせ
**`cnAnime` × `condManga`の1024が f1 0.2438**と、同ControlNetに
coarseを与えた0.2341を上回った。条件画像側にまだ伸びしろがある可能性。

#### 指標の落とし穴を2つ記録

1. **f1は紙の白さを見ていない**。`anime_base` 512/cs1.0はスイープ最良の
   f1 0.2263を、near_white 3.1%(灰色一色)で取っていた。細いストロークが
   GTの線の近くに落ちれば加点されるため。以後f1は必ず
   near_white/midtoneと併記する。
2. **near_whiteも単独では読めない**。`manga_base` 1024/cs1.0のnear_white
   87.8%は`anime_base`(81.5%)より高いが、目視では剣が出ていない。
   「きれいだから白い」のではなく**「描けていないから白い」**
   (下の2×2の項を参照——当初これを「インクが足りない」と書いたが、
   実際は縞状に破綻していた)。`line_width_p50` 1.53(GT 3.72)が
   その兆候だった。線幅と目視を必ず併読すること。

#### 最重要: 最良構成はfine-tuneしていない

目視(`montage_best.png`)でも数値でも、**`anime_base` 1024/cs2.0**が最良。
白い紙に細い線が出て、剣タイルで剣が明確に読める。f1 0.2568は前trackの
11モデル最良(`manga_consistency` 0.2337)を上回る。**LoRAを一切
当てていない素のControlNet**である。

両LoRAは全解像度・全csで素のControlNetに劣り、目視では縞模様(anime_lora)や
灰色の塊(manga_lora)に破綻している。

ただしこれは月曜の1024学習を否定しない——我々のLoRAは**512という
壊れた土俵で学習された**ので、その補償を学んだ可能性が高い。
1024学習は「`anime_base` 1024/cs2.0 = 0.2568を超えられるか」という
明確な対戦相手を持つ実験になる。超えられなければ「fine-tuneせず
素のControlNetを使う」が結論になる。

残課題: `anime_base`のf1はcs2.0でまだ上昇中なので、真の最適csが未確定。
1024 × cs2.5/3.0/4.0 を追加測定中(素のベースラインは月曜の学習が
超えるべき相手なので、正確に決める必要がある)。

### 高cs拡張で最適点確定 — 解像度とcsは交互作用 (2026-09-06 10:18〜11:04)

1024 × cs2.5/3.0/4.0 を追加(12セル)。素のControlNetは月曜の学習が
超えるべき相手なので、最適csを正確に決める必要があった。

`anime_base` @1024(GT: ink 0.0353 / 線幅 3.72 / 白 94.8% / oe 0.7117):

| cs | f1 | ink_ratio | line_w_p50 | near_white | midtone |
|---:|---:|---:|---:|---:|---:|
| 0.5 | 0.1686 | 0.2285 | 2.08 | 11.3% | 30.8% |
| 1.0 | 0.2341 | 0.0644 | 1.91 | 3.0% | 29.2% |
| 2.0 | 0.2568 | 0.0392 | 2.62 | 81.5% | 9.3% |
| **2.5** | **0.2582** | 0.0447 | **3.06** | 77.6% | 9.6% |
| 3.0 | 0.2577 | 0.0548 | 3.01 | 74.2% | 11.9% |
| 4.0 | 0.2494 | 0.0671 | 3.22 | 71.9% | 14.7% |

**f1はcs2.0〜3.0で平坦**(0.2568〜0.2582、差0.0014は5サンプルではノイズ域)。
単一の最適点ではなくプラトー。目視
(`results/resolution_sweep_20260906/montage_anime_base_cs_plateau.png`)では
cs2.5〜3.0が最良で、cs4.0は黒い滲みが出始める(f1低下と一致)。
**採用は cs2.5**。

軸ごとに最適が違う点も記録: csを上げるほど線幅はGTに近づく
(2.62→3.22 / GT 3.72)が、インク量はGTから離れ(0.0392→0.0671 / GT 0.0353)、
紙の白も減る(81.5%→71.9%)。

#### 解像度とcsは交互作用する(単独では効かない)

`anime_base`の`gt_bsds_f1`:

| | cs1.0 | cs2.0 |
|---|---:|---:|
| res512 | 0.2263 | 0.2416 |
| res768 | 0.2231 | 0.2470 |
| res1024 | 0.2341 | **0.2568** |

**cs1.0では解像度を上げてもほぼ横ばい**、**cs2.0では単調に改善**。
グレー背景のときとまったく同じ構造。briefingの「まず1024で土俵を揃える」は
正しかったが、cs=1.0のまま1024にしても何も見えなかったはずである。
**この2つは同時に動かさないと効果が現れない。**

#### 最終順位(各モデル最良セル)

| model | res | cs | f1 | ink | 線幅 | 白 |
|---|---:|---:|---:|---:|---:|---:|
| **anime_base** | 1024 | 2.5 | **0.2582** | 0.0447 | 3.06 | 77.6% |
| manga_base | 1024 | 1.0 | 0.2121 | 0.0345 | 1.53 | 87.8% |
| manga_lora | 1024 | 2.0 | 0.1949 | 0.2986 | 15.13 | 13.8% |
| anime_lora | 512 | 0.5 | 0.1253 | 0.5400 | 13.68 | 39.6% |
| (GT) | | | | 0.0353 | 3.72 | 94.8% |

`manga_base`はcs上昇で急落(cs2.5で0.1325、cs4.0で0.0239)、`anime_lora`は
壊滅的(cs2.5で0.0133)。**csレバーに耐えるのは`anime_base`だけ**。

#### 解釈上の留保 — f1 0.2582が意味すること

`montage_anime_base_cs_plateau.png`の1列目(条件画像)を見ると、剣タイルでは
**すでに剣がきれいな線で描かれている**。`anime_base`がしているのは
大部分が**条件画像の描き直し**である。

比較としては公正(11モデル全部が同じ前処理済み条件画像を与えられている)で、
かつTrack Bの問いは「条件忠実度」なので測りたかった軸そのものでもある。
ただし解釈は正確に: **f1 0.2582は「SDXLが線画を生成できた」ではなく
「条件画像に忠実である」ことを示す**。GTとの残差は忠実度ではなく、
**線の取捨選択と強弱づけ**という別の仕事。出力が全体に薄く、下絵のアタリ線を
残しているのがその現れ。

### 月曜(2026-09-07)投入分の確定

`experiments/run_controlnet_lora_sdxl_1024_20260907.sh`(既定`VARIANT=anime`)。
キャッシュ生成→6ステップスモーク→10エポック本学習→csラダー評価まで連結。

- `VARIANT=anime`: `lineart_anime`が2つのControlNetのうち唯一条件を
  追随できる(`manga_line`は512/1024とも縞状に破綻)
- **素のControlNetを同一ジョブ内で再測定**して並走比較する。スイープの数値を
  引用せず再測定するのは、「測り方が違った」疑いを残さないため
- csラダーは 0.5/1.0/1.5/2.0/2.5/3.0。1.5は紙の白が3.0%→81.5%に跳ぶ
  未測定区間を埋めるため、2.0〜3.0はプラトーを覆うため
- 判定は`experiments/score_sdxl_1024_eval_20260907.py`が
...
## 完了: 1024でのLoRA再学習 = 失敗。素のControlNetを超えられず
   (2026-09-06 12:56開始 → 09-08 16:07完了、50.3時間)

`experiments/run_controlnet_lora_sdxl_1024_20260907.sh`。
`checkpoints/controlnet_lora_sdxl_anime_1024_20260907/final`、
評価 `results/controlnet_lora_sdxl_anime_1024_20260907_eval/`。

**学習は健全に完走した**(不具合ではない):
21,160ステップ(10エポック)を8.57s/step、**peak VRAM 8.20GiBで全期間安定**。
損失は前半平均0.0341 → 後半平均0.0302の微減。日曜午後の投入から
火曜夕方に完了、失敗マーカーなし。

### 結果: 全軸で素のControlNetに劣る

| | f1 | ink_ratio | line_w_p50 | near_white | midtone |
|---|---:|---:|---:|---:|---:|
| **bare** cs2.5 | **0.2582** | 0.0447 | **3.06** | **77.6%** | 9.6% |
| finetuned cs2.0(最良) | 0.1602 | 0.1781 | 13.87 | 13.6% | 68.0% |
| (GT) | — | 0.0353 | 3.72 | 94.8% | 1.8% |

delta = **-0.0980**。目視
(`results/controlnet_lora_sdxl_anime_1024_20260907_eval/montage_ft_vs_bare.png`)
も一致し、数値以上に厳しい: fine-tune列は全csで灰色〜暗い地に太い黒帯、
剣タイルで剣が消え、cs2.5/3.0では斑状のノイズに崩壊する。

**512学習とまったく同じ失敗モードが1024でも再現した。解像度は
fine-tuneの失敗の原因ではなかった。**

### 否定した仮説: 「学習ターゲットが1024でぼけている」

fine-tune出力のmidtoneが全csで60〜68%と異常に高く、中間調はぼけた輪郭の
指標なので、480pxの`data/line`を1024に拡大したターゲット自体が
ソフトになっているのではないかと疑った。学習40タイルで測定:

| | bg_mode | near_white | midtone |
|---|---:|---:|---:|
| `data/line` 480px native | 254 | 93.9% | 2.7% |
| 同 1024へ拡大(モデルが見る姿) | 254 | 93.8% | **3.1%** |

**ほぼ変化しない。** ターゲットは1024でもきれいな二値的線画のままで、
この仮説は棄却。原因はデータのぼけではない。

### このtrackの結論

**ε-MSEでのfine-tuneは、この対データでは素のControlNetを改善しない。**
根拠は独立した3ラン(512 anime / 512 manga / 1024 anime)が
**すべて同じ方向に劣化**していること。しかも損失は下がっているので、
**学習目的関数は満たされたまま、欲しいものが悪化している**。
これは[[project_diffusion_domain_goal]]に記録された
「一指標最適化→線画性崩壊」の反復パターンの新しい実例である。

到達点は**LoRAなしの`Eugeoter/noob-sdxl-controlnet-lineart_anime`を
1024・cs2.5で推論する構成**(f1 0.2582)。学習は不要。
GTとの残差は条件忠実度ではなく**線の取捨選択と強弱づけ**。

機構は未解明。「なぜ小さなLoRAが基盤ControlNetの条件追随を壊すのか」は
次の問いとして残る(学習時はcs=1.0・CFGなしでしか見ていないのに、
推論はcs2.5・guidance5.0で行う、という訓練/推論の乖離が候補だが未検証)。

### 後片付け

学習が正常完走したので`resume_state`(2.5GB、中断再開専用)を削除。
最終アダプタ44MBのみ保持。**旧512スクリプトにあったこの後片付けが
新スクリプトから抜けていたので、スクリプト側にも追加した**。

### 共通基盤への続報 (2026-09-08 19:2x)

`outbox/note_sdxl_finetune_result_20260908.md`を書き、共通基盤の`inbox/`に
配達。あわせて`doc/CURRENT.md`を更新してコミット
(`lineart@diffusion` `f3b9460`)。

前報(09-06)で「水曜に結果が出る」と予告した学習の結末を伝えるもの。
CURRENT.mdの2箇所(trackポインタ / Next Actions #2)は
「training in flight, lands Wednesday evening」のままだったので、
失敗の事実と数値に差し替えた。

**伝えた要点**:

1. 1024再学習は失敗(f1 0.1602 vs bare 0.2582、全軸で劣る)。学習自体は
   健全に完走しているので不具合ではない
2. 512の失敗モードが1024でそのまま再現 = **解像度は原因ではなかった**
3. ε-MSE fine-tuneはこの対データでは素のControlNetを改善しない
   (独立3ランが同方向に劣化、しかも損失は下がっている)
4. **ペアデータの役割が「学習データ」から「測定器」へ移った**。
   ただし**これはSDXL路線での測定でありTrack Aに自動適用はできない**と
   明示した
5. **すべて5タイルが根拠という留保を最も強く伝えた**。
   CURRENT.md側にも「**do not treat 0.2582 as settled yet**」と明記し、
...
### 条件画像の交差を実用域で測り直し — 自分で破っていた教訓 (2026-09-10)

`doc/initial_notice.md`の次の一手#5に「条件画像側に伸びしろ
(`cnAnime`×`condManga`の1024が f1 0.2438 > coarseの0.2341)」と書いていたが、
**その測定はcs1.0でしか回していなかった**。cs1.0では`anime_base`は
紙の白3%の灰色一色なので、あれは**灰色同士の比較**。
「cs=1.0だけで比較しない」という本プロジェクト最初の教訓を自分で破っていた。

スイープに`anime_base_condmanga`(ControlNet・解像度・cs・シードすべて
`anime_base`と同一、**条件画像だけ差し替え**)を追加して実用域で再測定:
...
**順位は完全に反転**(cs1.0の0.2438>0.2341 → 実用域では全csでcoarseが上)。

**ただし「condMangaが劣る」とは結論できない。** 差は0.006程度、紙の白さも
ほぼ同じ(77.6% vs 76.6%)。**目視では`condManga`の方が線画らしい**
(`results/resolution_sweep_20260906/montage_conditioning_at_operating_cs.png`
の剣タイル: 白い紙に剣が出て余計なアタリ線が少ない)。

理由は説明がつく。`condCoarse`は下絵の薄いストロークまで拾う前処理なので
**構成線が多く残り、GTの線の近くにも落ちてf1のrecallを稼ぐ**。
`condManga`はより選択的。**差が出ている軸は我々が「残差」と呼んできた
線の取捨選択そのもので、いまの指標(f1・ink_ratio・near_white)はどれも
それを捉えない。**

判断: **292タイル検証に条件画像の軸は足さない**。292枚分の`manga_line`
前処理には隔離venvの再構築が要る一方、いまの指標では優劣を判定できないため
コストに見合わない。取捨選択を測る指標ができたら戻ること。

10分の実験で、根拠のない宿題を1つ潰し、代わりに「測れていない軸がある」
という具体的な形を得た。

## 完了: ホールドアウト292タイル検証 — 5枚の結論は保たれた
   (2026-09-10 23:15開始 → 09-11 11:36完了、推論のみ)

`experiments/run_holdout_validation_20260912.sh`。292タイル × 4構成 = 1,168枚。
結果 `results/holdout_validation_20260912/`。

**採点スクリプトのバグで一度落ちた**(判定行でdict同士を比較、
`TypeError: '>' not supported between instances of 'dict' and 'dict'`)。
推論1,168枚はすべて成功しており、採点のみ再実行。修正済み。

### アンカー照合: 合格

診断5枚の再測定が過去値を再現(最大差0.0063)。条件画像は共通基盤の
roughから再生成しているので完全一致ではないが、段取りは同じと確認できた。

| 構成 | 今回 | 過去 | 差 |
|---|---:|---:|---:|
| bare cs2.5 | 0.2615 | 0.2582 | +0.0033 |
| bare cs2.0 | 0.2612 | 0.2568 | +0.0044 |
| bare cs3.0 | 0.2578 | 0.2577 | +0.0001 |
| ft cs2.0 | 0.1539 | 0.1602 | -0.0063 |

### 群A(192枚・同系統): 結論は保たれ、差はむしろ拡大

| 構成 | f1平均 | 中央値 | 標準偏差 | ink | 線幅 | near_white |
|---|---:|---:|---:|---:|---:|---:|
| **bare cs2.0** | **0.3028** | 0.3046 | 0.0938 | 0.0368 | 2.02 | 64.3% |
| bare cs2.5 | 0.3017 | 0.3019 | 0.0935 | 0.0462 | 2.17 | 67.5% |
| bare cs3.0 | 0.2977 | 0.2992 | 0.0932 | 0.0604 | 2.57 | 65.0% |
| ft cs2.0 | 0.1853 | 0.1880 | 0.0520 | 0.1336 | 6.68 | 11.9% |
| **GT(群A)** | — | — | — | 0.0452 | 2.54 | **92.4%** |

**delta -0.1175**(5枚では-0.108)。平均と中央値がほぼ一致しており、
二峰性で平均が化けてはいない。**「既製品が全学習済みモデルを上回る」は
5枚の偏りではなかった。**

最適csは2.5→**2.0**に動いた(3点とも0.005以内なのでプラトーの範囲内だが、
5枚で決めたcs2.5が最適とは限らない)。

**5枚では見えなかった悪い知らせ**: 群Aの`near_white`は64〜67%で、
GTの92.4%から大きく離れる。診断5枚では79.6%だったので、
**グレー残差は5枚が示していたより深刻**。

### 群B(100枚・別出典housei): 順位は同じ、絶対値は半減

| 構成 | f1平均 | ink | 線幅 | near_white |
|---|---:|---:|---:|---:|
| bare cs3.0 | 0.1611 | 0.0176 | 1.94 | 89.1% |
| ft cs2.0 | 0.0978 | 0.1237 | 12.44 | 3.4% |
| **GT(群B)** | — | **0.0546** | **7.59** | 91.0% |

**群ごとのGT基準を出したことで「一般化しない」の中身が判明した。
失敗の仕方が出典によって逆である**:

- **群A**: インク量はGTとほぼ一致(0.0462 vs 0.0452)、線幅も近い。
  **問題は紙の白さだけ**
- **群B**: **紙の白さは合っている**(89.1% vs 91.0%)のに、
  **インクがGTの1/3、線幅がGTの1/4**。houseiのGTは少数の太い線で
  構成されているのに、モデルは細い線を多数引いている

つまり群Bの低スコアは汎化の失敗というより、**線の太さがGTの分布に
追随していない**という具体的な症状。

### 目視 — 残差の正体が明確になった

`results/holdout_validation_20260912/montage_holdout.png`

**群A**: GTは**ペン入れ済みの完成線画**(顔・髪・服が太い黒線で描き分け
られている)。`bare`列は**アタリ線を全部残した鉛筆スケッチ**。構造は追えて
顔も判別できるが、線を選んでいない。**5枚のときより遥かにはっきり見える。**

**群B**: GTは少数の太い線だけ。`bare`は細い線を多数引き、
`housei_001_03_04`の行では**背景に原稿の文字らしきテクスチャを創作**
している(GTにはない)。

`ft`列は両群とも黒い塊と斑点で、数値通り破綻。

### 結論

1. **本題の答え: 5枚の結論は正しかった。** 素のControlNetが学習済みLoRAを
   上回るという判定は192枚でも覆らず、差は拡大した(-0.098 → -0.1175)
2. **ただし到達点の絶対値は過大評価だった。** グレー残差は5枚が示したより
   深刻(near_white 79.6% → 64〜67%、GTは92.4%)
3. **残差は「線の取捨選択と強弱づけ」であることが目視で確定。**
   出力は鉛筆スケッチ、GTはペン入れ線画
4. **出典によって失敗の仕方が逆**(群A=紙が灰色、群B=線が細すぎる)。
   単一の構成で両方を満たす調整は存在しない可能性がある

### ★重大な訂正: 素のControlNetは何も足していなかった (2026-09-11、ユーザー指摘)

ユーザーが目視から3点を指摘し、いずれも当たっていた。特に3点目は
**このtrackの結論そのものを覆した**。

#### 指摘3(最重要): 「bareの値が良いのは下絵からほとんど動いていないからでは」

測定した。**条件画像そのものをGTと突き合わせる**と:

| 群A(lineart, n=60) | vs条件画像 | vsGT | 前処理超え |
|---|---:|---:|---:|
| **前処理のみ** | 1.0000 | **0.3177** | — |
| bare cs2.0 | 0.8788 | 0.3027 | **-0.0150** |
| bare cs3.0 | 0.8374 | 0.2961 | -0.0216 |
| ft cs2.0 | 0.2583 | 0.1820 | -0.1357 |

| 群B(housei, n=60) | vs条件画像 | vsGT | 前処理超え |
|---|---:|---:|---:|
| **前処理のみ** | 1.0000 | **0.1522** | — |
| bare cs2.0 | 0.8074 | 0.1537 | +0.0015 |
| bare cs3.0 | 0.7854 | 0.1569 | +0.0048 |
| ft cs2.0 | 0.2096 | 0.0960 | -0.0562 |

**どの構成も前処理器を超えていない。** 群Aではbareが前処理を
**0.015悪化させている**。`vs条件画像`が0.79〜0.88というのは
「入力からほとんど動いていない」ことの直接の証拠。

**つまり「既製ControlNetが全学習済みモデルを上回った」の正体は、
`LineartDetector(coarse=True)`という前処理器が単体で上回っていた
ということ。** SDXLの拡散スタックは良くて無害、実際には微害。

さらに「条件画像からの距離」と「f1」が**単調に対応**している
(0.88→0.3027, 0.84→0.2961, 0.26→0.1820)。**このデータでのf1は、
大部分が「前処理器の出力をどれだけ忠実に写したか」を測っていた。**
モデルの問題であると同時に**指標の問題**である。

**ただし訂正の範囲は正確に**: `ft`が悪いという判定は生き残る。
`near_white` 11.9%(GT 92.4%)・`midtone` 67%という紙の軸の破綻は
条件画像からの距離とは無関係だから。崩れたのは
「素のControlNetが最良の構成」という主張の方。

またユーザー指摘の通り、**`ft`は純粋なノイズではない**。cs2.0の目視では
斑点に埋もれつつも下絵の線構造が見える。「無茶苦茶」ではなく
「生成はしているが紙と線の性質が壊れている」が正しい。

#### 指摘1: 群Aと群Bは出典違いではなく「別のタスク」

| | GT黒画素率 | うちベタ塗り(7x7侵食で残存) | ほぼ空白のタイル |
|---|---:|---:|---:|
| 群A lineart | 0.0452 | **5.9%** | 1% |
| 群B housei | 0.0546 | **21.9%** | **38%** |

群Aは**背景がほとんどないキャラクター線画**。群Bは**キャラも背景も小物も
混在し、インクの2割強がベタ塗り、タイルの38%がほぼ空白**。

**これで前回の読みの誤りが判明した。** 「houseiのGTは少数の太い線」と
書いたが、`line_width_p50` 7.59は**太い線ではなくベタ塗りを測っていた**。
線幅は線と塗りを区別できないという本プロジェクト自身のルールに、
自分で引っかかっていた。

正しい読み: モデルのink 0.0176がGTの0.0546に届かないのは「線が細い」
のではなく**ベタ塗りを描けない**から。目視(`housei_004_18_10`)で明確——
GTは目と大量のベタ髪、`bare`は目の輪郭を細線で描くだけで黒の塗り潰しが
一切ない。

houseiのf1のばらつきが大きい(sd 0.107 vs 群A 0.094)のも、38%の空白タイル
でf1が不安定になるため。さらに`bare`は**空白領域に格子状の原稿テクスチャを
創作する**(`housei_001_17_18`)ので、空白タイルでprecisionを落とす。

#### 指摘2: モンタージュのhousei行にキャラ下絵がなかった

リスト先頭4枚を機械的に採っていたため、背景・小物に偏っていた。
`MONTAGE_PICKS_B`として**ベタ塗りキャラ・背景・ほぼ空白**を含む
代表6枚を明示指定するよう`score_holdout_validation_20260912.py`を修正。

#### このtrackの結論(書き換え)

1. **到達点は「既製ControlNetの1024/cs2.5」ではなく
   「前処理器`LineartDetector(coarse=True)`単体」**。拡散モデルを通すと
   群Aではわずかに悪化する
2. **`gt_bsds_f1`はこのタスクで前処理コピー度を測ってしまう**。
   モデルの寄与を見るには**条件画像自体のスコアを基準線として必ず併記**
   しなければならない。これは新しい運用ルール
3. ε-MSE fine-tuneが紙と線の性質を壊すという判定は維持
4. 群Aと群Bは別タスクなので、**平均してはいけない**。
   ベタ塗りを描けないことは群Bでのみ効く独立した欠陥

### 共通基盤へ訂正を送付 (2026-09-11)

`outbox/note_sdxl_correction_preprocessor_20260911.md`を書き、共通基盤の
`inbox/`に配達。`doc/CURRENT.md`のtrackポインタを訂正し、**lesson 6**を
追加してコミット(`lineart@diffusion` `2ba9a82`)。

09-06と09-08の2通で「素のControlNetが0.2582で11モデル表を上回る」と
伝えてしまっていたので、その訂正。CURRENT.mdにもその形で記載されていた。

**lesson 6として一般化した**(機構はSDXL固有ではないため):

> `gt_bsds_f1`を報告するときは条件画像自体のGTスコアを基準線として
> 必ず併記する。線画前処理で条件づけたControlNetでは、f1が
> 「前処理器の出力をどれだけ忠実に写したか」を測ってしまい、
> **何もしないモデルほど高く出る**。基準線は安価(推論不要、
> 手元の条件画像を採点するだけ)。
> 併せて、`lineart`系と`housei`系は別タスクなので**平均しない**。

**Track Aへの申し送りを明示した(本通達で最も重要な部分)**:
Track Aの0.2354も同じ構図にあり、**まだ誰も汚染を確認していない**。
ただし本trackは「Track Aは耐える可能性が高い」と見ている——
consistency損失は出力をGTのエッジへ動かす項で、`near_white`が
0.400→0.779と**条件画像コピーでは説明できない方向**に改善しているため。
だとすればTrack Aは**このプロジェクトで初めて「モデルが寄与している」
ことを示せる事例**になるので、測る価値はむしろ高い。

**取り下げない範囲も明記した**: fine-tuneが劣るという判定は維持
(292タイルでdelta -0.1175、`near_white` 11.9% vs GT 92.4%は
条件画像からの距離の副作用ではない)。

### 汚染チェックをTrack Aにも実施 — 全モデルが前処理器に負けている (2026-09-11)

ユーザー指示により、lesson 6の汚染チェックを本trackが代行。推論不要、
既存出力と条件画像の突き合わせのみ。

#### Track A(SD1.5 consistency)の結果

| 構成 | vs条件画像 | vsGT | 前処理超え |
|---|---:|---:|---:|
| **前処理のみ(manga_line)** | 1.0000 | **0.2566** | — |
| consistency w=0.02 cs2.5 | 0.5009 | 0.2175 | -0.0391 |
| consistency w=0.1 cs2.5 | 0.4866 | 0.2336 | -0.0230 |
| **w=0.2 cs2.5(新最良)** | 0.4618 | 0.2354 | **-0.0212** |
| w=0.5 cs2.5 | 0.5156 | 0.2226 | -0.0340 |

**Track Aの全構成も前処理器を下回る。**

#### ただし汚染の様相はSDXLと正反対

| | vs条件画像 |
|---|---:|
| SDXL bare | **0.88**(ほとんど写している) |
| SD1.5 consistency | **0.46**(生成している) |

さらに重みを上げると**条件画像から離れながらGTに近づく**
(w0.02→0.2で vs条件 0.5009→0.4618、f1 0.2175→0.2354)。
**SDXLでは「離れるほどf1が下がる」だったが、Track Aでは逆。
consistency損失は設計通りに働いており、方向は正しい。**
まだ前処理器の水準に届いていないだけである。

#### 同一5枚での統一比較(初めて公正に並べた)
...
| 前処理 `lineart_anime` 単体 | 0.2396 |
| SD1.5 consistency w0.2 cs2.5 | 0.2354 |
| SDXL ft 1024/cs2.0 | 0.1539 |

**本プロジェクトが学習した全モデルのうち、前処理器単体を超えたものは
一つもない。** 首位は`LineartDetector(coarse=True)`。

前処理器3種の間にも差がある(coarse 0.2639 > manga_line 0.2566 >
lineart_anime 0.2396)ので、**前処理器の選択自体が、これまでのモデル間の
差より大きい効果を持っていた**可能性がある。

#### 解釈上の注意

この表は「学習が無意味」を意味しない。前処理器は**下絵を線画へ変換する
決定的な関数**であって、それ自体がタスクの大部分を解いている。
問われているのは**前処理器の出力からGTへ残る差**(線の取捨選択・強弱・
ベタ塗り)を埋められるかで、**その差を埋めた実績がまだ一つもない**という
のが正確な現状。

Track Aの「離れながら近づく」挙動は、**その差を埋めに行っている唯一の
事例**であり、本trackのε-MSEとは質的に違う。

## ★残差の分解と選択天井 — このtrackの問いの終着点 (2026-09-11)

汚染チェックで「前処理器を超えたモデルが一つもない」と分かった後、
**その残差が何でできているか**を分解した。CPUのみ。
...
- **群A**: 前処理器はGTの線の58%を既に見つけているが、インクをGTの
  **1.4倍**描いている。→ 残差は**取捨選択(削る)**
- **群B**: 前処理器のインクはGTの**26%しかない**。不足分の正体はベタ塗りで、
  **前処理器のベタ率は0.0%**(エッジ検出器なので構造的に塗れない)。
  → 残差は**塗りを足す**

(注: 条件画像は白線/黒地なので、インク系の測定は極性を反転する必要がある。
最初`ink = gray<128`のまま測ってベタ率82〜95%という無意味な値を出した。
`edge_map`はCannyで極性不変なのでf1/precision/recallは影響を受けない。)

### 選択天井: 削るだけで 0.32 → 0.74

前処理器の線のうちGTとマッチしたものだけを残す**削除オラクル**で採点:

| プール | 前処理器 f1 | **選択天井 f1** | precision | recall |
|---|---:|---:|---:|---:|
| **A lineart** | 0.3231 | **0.7425** | 1.0 | 0.604 |
| B housei | 0.1574 | 0.3291 | 1.0 | 0.211 |

(前処理器の値が別実装の測定0.3177/0.3194と0.005以内で一致するので、
照合処理は既存指標と整合している)

**群Aでは、余計な線を削るだけでf1が2.3倍になる。** 本プロジェクトが
学習した全モデルは0.30前後で競っていたが、**削除オラクルは0.74**。
**残差の圧倒的大部分は選択問題である。**

recallが0.604で頭打ちなのは、GTの線の4割が前処理器出力に最初から
存在しないため。これが選択だけで到達できる上限。

群Bの天井が0.33止まりなのはrecallが0.21しかないから——
**ベタ塗りは選択では生み出せない**。

### このtrackの問いは終わった

Track Bの問いは「条件忠実度(SDXLは下絵から乖離するか)」だった。
**答えは出た——SDXLは乖離するどころか忠実すぎて条件画像を写している**
(`vs条件画像` 0.88)。**忠実度は最初から問題ではなかった。**

そして残差の正体は**「削る」と「塗る」**で、どちらも生成拡散モデルが
得意な仕事ではない。群A(我々の学習・評価データが両方ここにある)で
必要なのは**線ごとの残す/捨てるという判別的な判断**であり、
26億パラメータの拡散モデルは実際に何もしていない(0.88でコピー)か
壊している(ft)かのどちらかだった。

### 次に作るべきものへの含意

1. **群A向け: 判別的なストローク選択器**。入力は前処理器の出力(生の下絵
   ではない)、出力は各ストロークの残す/捨てる。ペアデータから教師信号が
   直接作れる(GTとマッチしたか否か)。**天井0.74は、現行最良0.30の2.3倍**
2. **群B向け: 領域塗り**。エッジ検出器が原理的にできない仕事なので、
   選択器とは別の機構が要る。ただし群Bは一度も学習しておらず、
   目標にするかは未決定(`doc/pool_inventory.md`)
3. **SDXLへのconsistency移植は保留を推奨**。Track Aが同じ機構を安い
