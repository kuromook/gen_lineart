...
1. **1024で再学習・再推論**(最優先)。VRAM 12GBで1024のSDXL ControlNet
   ...
   バッチサイズ・grad accumの調整。
   **所要時間は制約にならない**(ユーザー方針2026-09-06: 月〜木の4日連続
   バッチが標準の実行枠。数日かかる学習は許容範囲)。判断すべきは
   VRAMに収まるかと、途中クラッシュから復帰できるか。1024が
   **VRAM的に**不可能と判明した場合に限り、「学習512/推論1024」など
   段階的な切り分けを検討する——実行時間を理由に512へ後退しないこと。
