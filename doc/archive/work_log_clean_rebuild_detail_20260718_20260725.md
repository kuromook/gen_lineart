# Archived Work Log Detail - Clean Rebuild 2026-07-18 to 2026-07-25

This file preserves the detailed chronological `doc/work_log.md` content before the 2026-07-25 compaction.

The active work log now keeps only current pointers, compact summaries, and recent active history.
...
### 2026-07-24: hamlabi Auto-Review 398-Pair Training Start

Current goal:

- use the new variable-aspect search, post-align, and valid-mask workflow to
  train on a larger high-quality hamlabi pair set
- start with a 480px UNet run to verify whether the 398 reviewed pairs improve
  over the earlier 34-pair experiments

Pair generation result:

- background search/finalizer completed
- raw loop candidates: 1180
- auto-selected after deterministic filtering and deduplication: 400
- user visual review removed ranks:
  - 116
  - 118
- final current manifest: 398 pairs

Key files:

- auto candidates:
  - `results/region_search_loop/hamlabi_auto_candidates.csv`
- 400-pair masked manifest:
  - `dataset/regions_hamlabi_loop_auto_review_postalign12_masked_line_conservative/manifest.csv`
- filtered 398-pair manifest:
  - `dataset/regions_hamlabi_loop_auto_review_postalign12_masked_line_conservative_filtered398/manifest.csv`
- removed pairs record:
  - `dataset/regions_hamlabi_loop_auto_review_postalign12_masked_line_conservative_filtered398/removed_user_mismatch.csv`
- QC reviewed by user:
  - `dataset/regions_hamlabi_loop_auto_review_postalign12/post_align_qc.png`
  - `dataset/regions_hamlabi_loop_auto_review_postalign12_masked_line_conservative/valid_mask_qc.png`

Verification:

- loader reads the filtered manifest
- dataset length: 398
- loader returns `(rough, target, valid_mask)`
- tensor shapes at 480px:
  - `(1, 480, 480)`
  - `(1, 480, 480)`
  - `(1, 480, 480)`
- ranks 116 and 118 are absent
...
- unit: `hamlabi-filtered398-unet480-e40.service`
- notify watcher:
  - `hamlabi-filtered398-unet480-e40-notify.service`
- model: `unet`
- manifest:
  - `dataset/regions_hamlabi_loop_auto_review_postalign12_masked_line_conservative_filtered398/manifest.csv`
- image size: 480
...
- epochs: 40
- checkpoint dir:
  - `checkpoints/hamlabi_filtered398_unet480_e40/`
- log:
  - `logs/hamlabi_filtered398_unet480_e40.log`
- completion notification marker:
  - `checkpoints/hamlabi_filtered398_unet480_e40/epoch040.pth`

Current progress at save:
...
- wait for ntfy completion notification
- generate inference/montage from `epoch040.pth`
- compare against earlier 34-pair cleanup/UNet trials
- if visual output is promising, run a longer controlled training or test 768px
  / variable-aspect loader policy

### 2026-07-25: hamlabi Filtered398 UNet480 e40 Review

The `hamlabi_filtered398_unet480_e40` run completed after the previous save.
...
- log:
  - `logs/hamlabi_filtered398_unet480_e40.log`
- review outputs:
  - `results/hamlabi_filtered398_unet480_epoch040_outputs/`
  - `results/hamlabi_filtered398_unet480_epoch040_compare.png`

Training result:

- completed 40 epochs
- loss decreased from `G=0.5346` to `G=0.2151`

Tool fix:

- updated `tools/compare/make_region_manifest_compare.py` so region-manifest
  comparison now prefers `aligned_rough_path` / `aligned_line_path`, matching
  `lineart/region_dataset.py`

Output statistics over the 12-sample review montage:

| run | mean | std | black `<0.2` | white `>0.8` |
|---|---:|---:|---:|---:|
| `hamlabi_filtered398_unet480_epoch040` | 0.860 | 0.180 | 0.030 | 0.860 |
| `hamlabi_region_v2_unet480_epoch040` | 0.723 | 0.203 | 0.059 | 0.427 |
| `hamlabi_region_v2_unet480_epoch080` | 0.778 | 0.216 | 0.061 | 0.692 |

Visual interpretation:

- the larger 398-pair set improves dataset scale and keeps the pipeline
  working, but this plain U-Net e40 output is still not a clean hamlabi expert
- outputs are very white/soft overall and preserve large black-fill placement
  more than fine line structure
- many character lines collapse into blurred gray patches rather than crisp
  line art
- compared with the earlier 34-pair trial, the output is cleaner/whiter but
  not qualitatively better at line extraction

User visual review:

- given hamlabi's complex page information, rougher sketch tendency, and
  frequent black fills, this output is within the expected range
- unlike the previous separate dataset direction, persistent halo is not
  prominent here
- the output also does not appear to be escaping into broad all-over gray as
  much as feared
- the improved pair extraction is therefore considered effective, even though
  the model is not yet a finished line-art expert

Current decision:

- keep the 398-pair manifest as the current useful hamlabi dataset base
- apply the improved pair extraction workflow to other raw datasets, then train
  and evaluate them under comparable conditions
- do not judge the extraction strategy only from hamlabi, because hamlabi has
  unusually complex roughs and black-fill-heavy targets
- later hamlabi-specific modeling can still add line/fill separation,
  region-type splitting, or higher-resolution tests, but the broader next step
  is cross-dataset extraction/training evaluation

### 2026-07-25: Raw Dataset Zip Storage

Added uploaded ako5 v2 raw archive:

- `dataset/raw_zips/dataset_ako5ver2.zip`
  - zip root: `dataset_ako5`
  - manifest: `dataset_ako5/manifest.json`
  - rows: 53

Moved raw archives into canonical storage:

- `dataset/raw_zips/dataset_ako5ver2.zip`
- `dataset/raw_zips/dataset_hamlabi.zip`

Kept root-level compatibility symlinks:
...
- put newly uploaded raw dataset zip files under `dataset/raw_zips/`
- keep root symlinks only for script compatibility
- prefer canonical `dataset/raw_zips/...` paths in new commands and logs
- do not overwrite older raw archives when source versions differ

### 2026-07-25: ako5ver2 Region Pair Extraction

Ran improved region-matched pair extraction on the uploaded ako5 v2 archive.
...
- zip root: `dataset_ako5`
- manifest rows: 53
- usable rows present in zip: 47
- skipped missing manifest rows: 6
  - `page0004`
  - `page0022`
  - `page0029`
  - `page0031`
...
- `tools/pair_extraction/match_ako5_regions.py`
  - now skips manifest rows whose sketch/line files are missing from the zip
- `tools/pair_extraction/extract_ako5_region_tiles.py`
  - now applies the same available-file filtering when loading pages

Region matching command:

```bash
venv/bin/python tools/pair_extraction/match_ako5_regions.py \
  --zip dataset/raw_zips/dataset_ako5ver2.zip \
  --max-dim 1200 \
  --top-k 400 \
  --qc-count 80 \
  --csv-out results/ako5ver2_region_20260725_matches.csv \
  --json-out results/ako5ver2_region_20260725_matches.json \
  --qc-out results/ako5ver2_region_20260725_match_qc.png
```

Region result:

- full pair search: 47 x 47 page combinations
- raw candidates before dedupe/top filtering: 234
- saved region candidates: 82
- QC:
  - `results/ako5ver2_region_20260725_match_qc.png`
- region candidate metric ranges:
  - `score`: 3.307 to 6.009, median 4.657
  - `edge_f1`: 0.605 to 0.948, median 0.808
  - `chamfer`: 1.285 to 5.142, median 2.893

Tile dry-runs:

- broad dry-run:
  - accepted: 211
  - CSV: `results/ako5ver2_region_20260725_tiles.csv`
  - QC:
    - `results/ako5ver2_region_20260725_tiles_qc.png`
    - `results/ako5ver2_region_20260725_tiles_qc_tail.png`
    - `results/ako5ver2_region_20260725_tiles_qc_sample.png`
- strict dry-run:
  - thresholds:
    - `min_tile_score=3.4`
    - `min_f1=0.65`
    - `max_chamfer=6.5`
  - accepted: 88
  - CSV: `results/ako5ver2_region_20260725_tiles_strict.csv`
  - QC:
    - `results/ako5ver2_region_20260725_tiles_strict_qc.png`
    - `results/ako5ver2_region_20260725_tiles_strict_qc_tail.png`
    - `results/ako5ver2_region_20260725_tiles_strict_qc_sample.png`

Saved strict extraction:
...
- saved CSV:
  - `results/ako5ver2_region_20260725_tiles_strict_saved.csv`
- saved QC:
  - `results/ako5ver2_region_20260725_tiles_strict_saved_qc.png`
  - `results/ako5ver2_region_20260725_tiles_strict_saved_qc_tail.png`
  - `results/ako5ver2_region_20260725_tiles_strict_saved_qc_sample.png`

Saved set verification:
...
venv/bin/python tools/evaluation/audit_pair_dataset_integrity.py \
  --train-lists dataset/pairs_480/valid_train_ako5ver2_region_20260725_strict.txt \
  --eval-lists dataset/pairs_480/valid_test.txt dataset/pairs_480/eval_fixed_clean_lineart004.txt \
  --line-dir-override ako5r=dataset/pairs_480/train/line_ako5ver2_region_20260725_strict \
  --output-summary results/pair_dataset_integrity_summary_ako5ver2_region_20260725_strict.csv \
  --output-findings results/pair_dataset_integrity_findings_ako5ver2_region_20260725_strict.csv
```
...
- ako5ver2 region matching is now usable with the improved extraction flow
- the strict 88-pair set is small but clean enough for a first controlled
  training/evaluation pass
- broad 211-pair set is useful for review/expansion, but the strict set should
  be the first training candidate

Next action:

- train a short controlled model using the strict 88-pair list and explicit
  line-dir override/mix policy, then compare halo, gray escape, and line pickup
  against the hamlabi filtered398 result and previous ako bad-rough behavior

### 2026-07-25: ako5ver2 hamlabi-style Variable Region Extraction

Correction to the previous ako5 extraction interpretation:

- `ako5ver2_region_20260725_strict` was still a region-guided 480px square
  tile extraction
- it did not preserve variable-aspect parent regions or recursively search
  child regions the way the hamlabi workflow does

Changed direction:

- keep the 88-tile strict set only as a fixed-tile baseline
- run ako5 through the hamlabi-style flow:
  - line-anchored variable-aspect parent region proposals
  - rough-side translation/scale search
  - recursive child search inside large parent regions
  - variable-aspect materialized manifest
...
- `tools/pair_extraction/match_hamlabi_regions.py`
  - now skips missing manifest rows, so the same matcher can run on
    `dataset_ako5` archives with missing manifest entries
- `tools/pair_extraction/materialize_region_candidates.py`
  - new generic materializer for parent/child candidate CSVs
  - filters by `decision`
  - deduplicates by page plus rough/line box IoU
  - writes variable-aspect rough/line PNGs and CSV/JSON manifests

Parent region command:

```bash
venv/bin/python tools/pair_extraction/match_hamlabi_regions.py \
  --zip dataset/raw_zips/dataset_ako5ver2.zip \
  --zip-root dataset_ako5 \
  --region-close 121 \
  --line-margin 0.12 \
  --min-region-area 3500 \
  --min-region-size 180 \
  --max-region-ratio 0.85 \
  --max-regions-per-page 6 \
  ...
  --search-ratio 0.14 \
  --search-step 72 \
  --search-step-ratio 0.12 \
  --match-size 320 \
  --output-long-side 768 \
  --qc-count 100 \
  --csv-out results/ako5ver2_varregion_20260725_parent_light_candidates.csv \
  --json-out results/ako5ver2_varregion_20260725_parent_light_candidates.json \
  --qc-out results/ako5ver2_varregion_20260725_parent_light_qc.png
```
...
- candidates: 84
- QC:
  - `results/ako5ver2_varregion_20260725_parent_light_qc.png`

Child region command:

```bash
venv/bin/python tools/pair_extraction/refine_hamlabi_large_regions.py \
  --zip dataset/raw_zips/dataset_ako5ver2.zip \
  --zip-root dataset_ako5 \
  --parents results/ako5ver2_varregion_20260725_parent_light_candidates.csv \
  --parent-long-side 900 \
  --parent-area 550000 \
  --region-close 61 \
  --line-margin 0.14 \
  --min-region-area 900 \
  ...
  --match-size 320 \
  --output-long-side 768 \
  --min-match-score 0.42 \
  --min-f1 0.12 \
  --max-chamfer 28.0 \
  --min-rough-std 8.0 \
  --qc-count 100 \
  --csv-out results/ako5ver2_varregion_20260725_child_candidates.csv \
  --json-out results/ako5ver2_varregion_20260725_child_candidates.json \
  --qc-out results/ako5ver2_varregion_20260725_child_qc.png
```

Child result:

- large parents searched: 74
- child rows: 347
- child candidates: 217
- QC:
  - `results/ako5ver2_varregion_20260725_child_qc.png`

Variable-aspect materialization:

```bash
venv/bin/python tools/pair_extraction/materialize_region_candidates.py \
  --zip dataset/raw_zips/dataset_ako5ver2.zip \
  --zip-root dataset_ako5 \
  --csv \
    results/ako5ver2_varregion_20260725_parent_light_candidates.csv \
    results/ako5ver2_varregion_20260725_child_candidates.csv \
  --decision candidate \
  --out-base dataset/regions_ako5ver2_varregion_20260725 \
  --manifest-out dataset/regions_ako5ver2_varregion_20260725/manifest.json \
  --csv-out dataset/regions_ako5ver2_varregion_20260725/manifest.csv \
  --long-side 768 \
  --name-prefix ako5vr \
  --duplicate-iou 0.72
```
...
- variable-aspect pairs: 291
- manifest:
  - `dataset/regions_ako5ver2_varregion_20260725/manifest.csv`
  - `dataset/regions_ako5ver2_varregion_20260725/manifest.json`

Post-align:

- output:
  - `dataset/regions_ako5ver2_varregion_20260725_postalign12/`
- rows: 291
- shifted: 264
- mean score gain: 0.1980
- QC:
  - `dataset/regions_ako5ver2_varregion_20260725_postalign12/post_align_qc.png`
...
- mask stats:
  - `ignore_ratio`: min 0.0000, median 0.0152, max 0.7074
  - `unsupported_line_edge_ratio`: min 0.0000, median 0.3173, max 0.9420

Interpretation:

- ako5 is now processed through the same broad workflow as hamlabi, not only
  fixed 480px tiles
- the parent/child search produces a sizable variable-aspect candidate set
- top QC rows look directionally useful, but the masked QC shows that some
  candidates still have large unsupported target areas
- the 291-pair manifest is review-ready, not automatically promoted as a clean
  training set

Next action:

- filter/review the 291 variable-region candidates before training
- likely first filter by high mask validity / low unsupported line ratio, then
  create a smaller reviewed ako5 variable-region training manifest

### 2026-07-25: ako5ver2 User Review Flags

Recorded user visual review decisions for:
...
Held out as mask-insufficient umbrella cases:

- review indices: 69, 79, 86
- reason: rough contains umbrella but line art does not; likely added later by
  3D or separate layer, so the current conservative valid mask is insufficient
- rows written to:
...
Created filtered manifest excluding both mismatch and mask-insufficient hold
rows:
