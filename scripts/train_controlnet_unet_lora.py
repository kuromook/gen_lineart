"""Hypothesis #6 (inbox/initial_notice.md, 2026-09-05): every experiment in
this track so far -- data pool (#1), LoRA rank (#2), epoch count (#3),
x0-consistency loss (#4), caption vocabulary (#5) -- kept the base UNet
completely frozen (`unet.requires_grad_(False)` in the shared
../lineart/scripts/train_controlnet.py, with LoRA added to the ControlNet
only). So the UNet's own text-conditioned denoising prior, which is where
SD1.5's hatching/screentone associations actually live, has never been
trainable here. That is the leading explanation for why #5 (removing
"manga panel" from every training caption) not only failed but came out
slightly worse: changing the text at training time teaches nothing when
the module that reads the text is frozen.

This script is ../lineart/scripts/train_controlnet.py with a second LoRA
adapter added to the base UNet, trained jointly with the ControlNet LoRA
on the same paired rough->line data, so the text-conditioned pathway can
actually move toward the target distribution. The UNet LoRA config
(rank, alpha, gaussian init, to_k/to_q/to_v/to_out.0) and its save format
(StableDiffusionPipeline.save_lora_weights, which infer_controlnet.py's
existing --lora-dir already loads) both follow
../lineart/scripts/train_domain_lora.py, which has been training UNet
LoRAs in this codebase since 2026-08-04.

--unet-lora-rank 0 (the default) reproduces plain train_controlnet.py
exactly, including keeping the frozen UNet in fp16, so this script doubles
as its own A/B harness -- same design as train_controlnet_consistency.py's
--consistency-weight 0. When the rank is non-zero the UNet must stay fp32
and go through accelerator.prepare() instead (mixed-precision training
cannot keep fp16 master weights for trainable params); that branch is the
one real structural difference from the frozen-UNet path.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="if non-zero, also train a LoRA adapter of this rank on the base UNet's attention "
        "layers, jointly with the ControlNet LoRA (hypothesis #6 -- see module docstring). "
        "0 keeps the UNet fully frozen and fp16, reproducing plain train_controlnet.py.",
    )
...
def main():
    ...
    if args.unet_lora_rank:
        unet_lora_config = LoraConfig(
            r=args.unet_lora_rank,
            lora_alpha=args.unet_lora_rank,
            init_lora_weights="gaussian",
            target_modules=["to_k", "to_q", "to_v", "to_out.0"],
        )
        unet.add_adapter(unet_lora_config)
        unet.train()
        n_unet_trainable = sum(p.numel() for p in unet.parameters() if p.requires_grad)
        print(
            f"[train_controlnet] UNet LoRA mode: rank={args.unet_lora_rank}, "
            f"{n_unet_trainable:,} trainable params (base UNet frozen)"
        )
    ...
    if not args.unet_lora_rank:
        # Frozen UNet is inference-only here, so fp16 is safe and halves its
        # footprint. With a trainable adapter it must stay fp32 and go
        # through accelerator.prepare() instead (mixed precision keeps fp32
        # master weights; fp16 params would break gradient unscaling).
    ...
    if args.unet_lora_rank:
        controlnet, unet, optimizer, dataloader = accelerator.prepare(
            controlnet, unet, optimizer, dataloader
        )
    else:
    ...
    def save_unet_lora(path):
        """Saved in diffusers' standard LoRA layout (same call as
        train_domain_lora.py) so infer_controlnet.py's existing --lora-dir
        loads it with no changes."""
    ...
    def save_eval_snapshot(step):
        ...
        if args.unet_lora_rank:
            save_unet_lora(os.path.join(light_path, "unet_lora"))
    ...
    accumulate_models = (controlnet, unet) if args.unet_lora_rank else (controlnet,)
    ...
    if args.unet_lora_rank:
        save_unet_lora(os.path.join(final_path, "unet_lora"))
        print(f"[train_controlnet] saved UNet LoRA to {os.path.join(final_path, 'unet_lora')}")
