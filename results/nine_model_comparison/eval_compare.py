"""Nine-way comparison, extending results/eight_model_comparison/eval_compare.py
with the x0-vs-GT Sobel consistency-loss run (hypothesis #4,
inbox/initial_notice.md 2026-09-03/04): same base_ckpt/ControlNet-init/
conditioning/rank/epochs/lr as manga_trained (controlnet_lora_manga_20260827,
rank16), plus an auxiliary edge-consistency loss term on the decoded x0
estimate at low timesteps, to test whether a loss-function-side
intervention (rather than data pool, LoRA rank, or epoch count -- all
already rejected) can suppress the cross-hatch fallback.
- manga_consistency_trained: controlnet_lora_manga_consistency_20260904
"""
...
def main():
    ...
    import csv
    with open(TRACK / "results/nine_model_comparison/metrics.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)
    print(f"\nsaved: {TRACK / 'results/nine_model_comparison/metrics.csv'}")
