"""Ten-way comparison, extending results/nine_model_comparison/eval_compare.py
with the "manga panel" caption-ablation retrain (hypothesis #5,
inbox/initial_notice.md 2026-09-05): same base_ckpt/ControlNet-init/
conditioning/rank/epochs/lr as manga_trained (controlnet_lora_manga_20260827,
rank16), but every one of the 8467 training captions has "manga panel"
stripped (data/captions_no_manga_word.csv) and the eval caption matches --
to test whether the base SD1.5 model's pretrained "manga" concept prior
(hatching/screentone-heavy per literature review) was competing with the
LoRA's correction throughout training, not just at inference time (a
same-checkpoint inference-caption-only ablation on 2026-09-05,
results/caption_ablation_20260905/, was confounded/inconclusive).
- manga_nomangaword_trained: controlnet_lora_manga_nomangaword_20260905
"""
...
MODELS = {
    ...
    "manga_consistency_trained": TRACK / "results/controlnet_lora_manga_consistency_20260904_eval",
    "manga_nomangaword_trained": TRACK / "results/controlnet_lora_manga_nomangaword_20260905_eval",
}
...
def main():
    ...
    import csv
    with open(TRACK / "results/ten_model_comparison/metrics.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)
    print(f"\nsaved: {TRACK / 'results/ten_model_comparison/metrics.csv'}")
