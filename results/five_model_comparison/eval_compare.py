"""Five-way comparison, extending results/four_model_comparison/eval_compare.py
with the SDXL migration candidate (candidate #1 from the 2026-08-27 literature
survey), all against GT on diag5:
...
- sdxl_trained: controlnet_lora_sdxl_20260829 (candidate #1: SDXL migration,
  ...
  lineart_coarse preprocessing, 2 epochs)
"""
...
def main():
    ...
    import csv
    with open(TRACK / "results/five_model_comparison/metrics.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)
    print(f"\nsaved: {TRACK / 'results/five_model_comparison/metrics.csv'}")
