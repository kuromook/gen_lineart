"""Hatch-fill-escape metric: orientation_entropy (already in
tile_region_manifest_480.py) measures whether an edge map's stroke
directions are dominated by a small number of repeated angles (classic
rigid crosshatch -> LOW entropy) versus spread across many directions
(genuine content or at least non-rigid hallucination -> HIGHER entropy).
This is orthogonal to gt_bsds_f1/precision/recall, which only measure
positional agreement with GT and say nothing about *what kind* of
hallucination is present. Goal right now is specifically "escape hatch-fill
hallucination", so this is the metric that actually tracks that axis.

orientation_entropy is a 0..1 quantity (0 = all edge energy in one 15-deg
bin, i.e. maximally hatch-like; 1 = perfectly uniform across all 12 bins).
Reported both per-output and against GT's own value as a reference ceiling
(GT is real linework, not hatch, so its entropy is the "healthy" range to
