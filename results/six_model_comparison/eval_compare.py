"""Six-way comparison, extending results/five_model_comparison/eval_compare.py
with the SDXL manga_line retrain (candidate #1 follow-up), all against GT on diag5:
...
- sdxl_trained: controlnet_lora_sdxl_20260829 (candidate #1: SDXL migration,
  animagine-xl-3.1 base + noob-sdxl-controlnet-lineart_anime init +
  lineart_coarse preprocessing, 2 epochs) -- worst of the 5 on both axes
- sdxl_manga_trained: controlnet_lora_sdxl_manga_20260830 (candidate #1
  follow-up: same base, but noob-sdxl-controlnet-manga_line init +
  data/rough_manga_line conditioning, name-matched to the ControlNet's
  implied training preprocessor, 2 epochs)
"""
...
def main():
    ...
    import csv
    with open(TRACK / "results/six_model_comparison/metrics.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)
    print(f"\nsaved: {TRACK / 'results/six_model_comparison/metrics.csv'}")
