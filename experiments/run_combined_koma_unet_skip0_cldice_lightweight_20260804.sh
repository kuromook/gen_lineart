#!/bin/bash
# clDice follow-up A: halved weight, no ink counter-term.
#
...
# hard_cldice (the metric it was trained for, evaluated hard-threshold)
# but raised raw endpoint_count and ink_ratio ~2x GT -- the model appears
# to satisfy topology sensitivity partly by painting wider/darker rather
# than tracing more precisely. This run tests whether simply halving
# cldice_weight (0.3 -> 0.15) reduces that over-inking side effect while
# still retaining most of the topo_sensitivity gain, as a baseline before
# trying an explicit counter-term (see the _inkpenalty sibling script).
#
# Mirrors run_combined_koma_unet_skip0_cldice_finegrid_20260804.sh exactly
# except CLDICE_WEIGHT default and TAG.
set -euo pipefail
cd "$(dirname "$0")/.."

PY=./venv/bin/python
TAG=combined_koma_unet_skip0_cldice_lightweight_20260804
EPOCHS=${EPOCHS:-60}
SAVE_EVERY=${SAVE_EVERY:-5}
CLDICE_WEIGHT=${CLDICE_WEIGHT:-0.15}
CLDICE_ITERS=${CLDICE_ITERS:-10}
INK_WEIGHT=${INK_WEIGHT:-0.0}
...
echo "[$(date --iso-8601=seconds)] combined_koma unet_skip0+cldice(lightweight) fine-grid (5..${EPOCHS} step ${SAVE_EVERY}) start"
...
echo "=== train unet_skip0 + cldice(lightweight), ${EPOCHS} epochs, checkpoint every ${SAVE_EVERY} ==="
...
experiments/send_autoloop_notification.sh \
  "Lineart combined_koma unet_skip0+cldice(lightweight) fine-grid (5..${EPOCHS} step ${SAVE_EVERY}) trajectory complete" \
  "Review $MONTAGE, $FIXED_CSV, $STAB_CSV" || true

echo "done marker: $DONE"
echo "[$(date --iso-8601=seconds)] combined_koma unet_skip0+cldice(lightweight) fine-grid trajectory complete"
