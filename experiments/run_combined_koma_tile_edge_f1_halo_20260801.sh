#!/bin/bash
# Re-test the agreement-halo hypothesis again, this time splitting on the
# per-tile edge_f1 already computed by the koma tiling pipeline
# (tools/pair_extraction/tile_region_manifest_480.py -> results/{source}/
# {source}_koma_tiles_480_20260729.csv) instead of a generic post-hoc
# agreement score (tools/evaluation/score_pair_agreement.py, used in the
# 2026-08-01 combined_koma_agreement_halo run).
#
# Why this is a cleaner test: the earlier finding was that most of the old
# (2026-07-20, pre-alignment-fix) agreement-halo effect was a byproduct of
# raw XY-coordinate misalignment, not intrinsic rough/line content
# ambiguity, and the same-day rerun on aligned combined_koma_20260729 data
# with a generic agreement score showed the effect had mostly vanished
# (F1 gap 0.009). But score_pair_agreement.py doesn't know anything about
# how much alignment correction a tile already received -- edge_f1 in the
# tile CSVs is measured *after* the pipeline's own coarse-to-fine alignment
# refinement, so splitting on it should more directly isolate residual
# content-correspondence ambiguity (or reveal that even that residual is
# small, which would be an even stronger negative result).
#
# Speed trick: reuses the already-materialized atari/lucy_mild aux hints
# from the 2026-08-01 noadv ablation run instead of regenerating them.
set -euo pipefail
cd "$(dirname "$0")/.."

PY=./venv/bin/python
TAG=combined_koma_tile_edge_f1_halo_20260801
EPOCHS=${EPOCHS:-2}
SPLIT_COUNT=${SPLIT_COUNT:-450}
SCORE_FIELD=${SCORE_FIELD:-edge_f1}
TILE_CSVS=(
  results/ako5ver2/ako5ver2_koma_tiles_480_20260729.csv
  results/housei/housei_koma_tiles_480_20260729.csv
  results/hamlabi/hamlabi_koma_tiles_480_20260729.csv
  results/fitness/fitness_koma_tiles_480_20260729.csv
  results/gakuen/gakuen_koma_tiles_480_20260729.csv
)
LINE_DIR=dataset/pairs_480/train/line_combined_koma_20260729
EVAL_LIST=dataset/pairs_480/eval_clean_lineart004_8.txt
EVAL_RAW_ROUGH=dataset/pairs_480/test/rough
TRAIN_AUX=results/combined_koma_lucy_mild_noadv_20260801_lucy_mild_train
EVAL_AUX=results/combined_koma_lucy_mild_noadv_20260801_lucy_mild_eval
BASELINE_MODEL_NAME=combined_koma_lucy_mild_msgan_20260729
BASELINE_OUT=results/${BASELINE_MODEL_NAME}

LOG=logs/${TAG}.log
DONE=logs/${TAG}.done

SCORE_CSV=results/${TAG}_scores.csv
HIGH_LIST=dataset/pairs_480/${TAG}_high${SPLIT_COUNT}.txt
LOW_LIST=dataset/pairs_480/${TAG}_low${SPLIT_COUNT}.txt
METRICS=results/fixed_output_metrics_${TAG}_compare.csv
HALO_METRICS=results/halo_metrics_${TAG}_compare.csv
MONTAGE=results/compare_${TAG}.png

mkdir -p logs results
: > "$LOG"
exec > >(tee -a "$LOG") 2>&1

echo "[$(date --iso-8601=seconds)] combined_koma tile edge_f1 halo re-test start"
echo "tag=$TAG epochs=$EPOCHS split_count=$SPLIT_COUNT score_field=$SCORE_FIELD reused_aux=$TRAIN_AUX"

if [[ ! -d "$TRAIN_AUX" || ! -d "$EVAL_AUX" ]]; then
  echo "FATAL: expected pre-materialized aux dirs missing: $TRAIN_AUX / $EVAL_AUX" >&2
  exit 1
fi

echo "=== split by pipeline-computed post-alignment $SCORE_FIELD ==="
"$PY" tools/evaluation/split_koma_by_tile_edge_f1.py \
  --tile-csvs "${TILE_CSVS[@]}" \
  --score-field "$SCORE_FIELD" \
  --count "$SPLIT_COUNT" \
  --high-list "$HIGH_LIST" \
  --low-list "$LOW_LIST" \
  --output-csv "$SCORE_CSV"

train_candidate() {
  local label=$1
  local file_list=$2
  local ckpt=checkpoints/${TAG}_${label}
  local out=results/${TAG}_${label}
  mkdir -p "$out"
  echo "=== train ${label} ($(wc -l < "$file_list") tiles) ==="
  "$PY" scripts/train_i2i_survey.py \
    --checkpoint-dir "$ckpt" \
    --file-list "$file_list" \
    --line-dir "$LINE_DIR" \
    --aux-dir "$TRAIN_AUX" \
    --epochs "$EPOCHS" \
    --workers 0 \
    --model cleanup \
    --gan --multiscale-gan \
    --lr 8e-5 --lr-d 2e-5 --pos-weight 5.0 \
    --bce-weight 0.75 --l1-weight 0.03 \
    --shape-weight 0.08 --ink-weight 0.14 \
    --binary-weight 0.10 --structure-weight 0.04 \
    --adv-weight 0.03 --feature-match-weight 0.08 \
    --require-cuda

  echo "=== infer ${label} on eval list ==="
  "$PY" scripts/inference_i2i_batch.py \
    --checkpoint "$ckpt/best.pth" \
    --file-list "$EVAL_LIST" \
    --rough-dir "$EVAL_RAW_ROUGH" \
    --aux-dir "$EVAL_AUX" \
    --output-dir "$out" \
    --autocontrast
}

train_candidate high_edgef1 "$HIGH_LIST"
train_candidate low_edgef1 "$LOW_LIST"

echo "=== montage ==="
"$PY" tools/compare/make_multi_model_eval_compare.py \
  --sample-list "$EVAL_LIST" \
  --split test \
  --model lucy_mild_msgan_mixed="$BASELINE_OUT" \
  --model high_edgef1="results/${TAG}_high_edgef1" \
  --model low_edgef1="results/${TAG}_low_edgef1" \
  --output "$MONTAGE"

echo "=== fixed metrics ==="
"$PY" tools/evaluation/evaluate_fixed_outputs.py \
  --models \
    "$BASELINE_MODEL_NAME" \
    ${TAG}_high_edgef1 \
    ${TAG}_low_edgef1 \
  --sample-list "$EVAL_LIST" \
  --split test \
  --output-csv "$METRICS"

echo "=== halo metrics ==="
"$PY" tools/evaluation/evaluate_halo_outputs.py \
  --models \
    "$BASELINE_MODEL_NAME" \
    ${TAG}_high_edgef1 \
    ${TAG}_low_edgef1 \
  --sample-list "$EVAL_LIST" \
  --split test \
  --output-csv "$HALO_METRICS"

# CNN+GAN-family work runs on cleanup-refiner, but Monday's cron job
# (scripts/train_controlnet.py) only exists on diffusion-controlnet --
# switch back automatically since the user may be asleep when this finishes.
echo "=== switching back to diffusion-controlnet branch ==="
git checkout diffusion-controlnet

{
  echo "completed_at=$(date --iso-8601=seconds)"
  echo "tag=$TAG"
  echo "epochs=$EPOCHS"
  echo "split_count=$SPLIT_COUNT"
  echo "score_field=$SCORE_FIELD"
  echo "high_list=$HIGH_LIST"
  echo "low_list=$LOW_LIST"
  echo "train_aux=$TRAIN_AUX (reused)"
  echo "eval_aux=$EVAL_AUX (reused)"
  echo "scores=$SCORE_CSV"
  echo "metrics=$METRICS"
  echo "halo_metrics=$HALO_METRICS"
  echo "montage=$MONTAGE"
} > "$DONE"

experiments/send_autoloop_notification.sh \
  "Lineart combined_koma tile edge_f1 halo re-test complete" \
  "Review $MONTAGE, $METRICS, and $HALO_METRICS" || true

echo "done marker: $DONE"
echo "[$(date --iso-8601=seconds)] combined_koma tile edge_f1 halo re-test complete"
