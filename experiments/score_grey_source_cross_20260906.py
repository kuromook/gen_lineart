"""Score the 2x2 ControlNet x conditioning cross, and print it as a matrix on
the paper-white axis so the question it was built for -- does the grey wash
follow the ControlNet or the conditioning preprocessor? -- is answered by
reading a row against a column.
...
ROOT = TRACK / "results/grey_source_cross_20260906"
OUT_ROOT = ROOT / "outputs"
IMAGE_SIZE = 480
BSDS_TOLERANCE_PX = 2.0
CONTROLNETS = ["cnAnime", "cnManga"]
CONDITIONS = ["condCoarse", "condManga"]
...
def montage(resolutions):
    cell_px, label_h = 200, 24
    font = ImageFont.load_default()
    rows = [(cn, cond) for cn in CONTROLNETS for cond in CONDITIONS]
    cols = ["cond", "GT"] + [str(r) for r in resolutions]
    sheet = Image.new("L", (len(cols) * cell_px, len(rows) * (cell_px + label_h) + label_h), 255)
    draw = ImageDraw.Draw(sheet)
    for c, label in enumerate(cols):
        draw.text((c * cell_px + 4, 6), label if c < 2 else f"res{label}", fill=0, font=font)
    cond_dir = {"condCoarse": "data/diag_rough_lineart_coarse",
    ...
    for r, (cn, cond) in enumerate(rows):
        ...
        paths += [OUT_ROOT / f"{cn}_{cond}" / f"res{res}_cs1.0" / f"{GRID_SAMPLE}_out.png"
                  for res in resolutions]
        for c, p in enumerate(paths):
            if Path(p).exists():
                sheet.paste(Image.open(p).convert("L").resize((cell_px, cell_px)),
                            (c * cell_px, y0 + label_h))
    out = ROOT / "montage_grey_source.png"
    sheet.save(out)
    print(f"\nmontage: {out}")
...
def main():
        ...
        for cond in CONDITIONS:
            ...
            for cell in sorted(d.iterdir()):
                if not (cell / ".complete").exists():
                    continue
                res = int(cell.name.split("_")[0][3:])
                rows.append({"controlnet": cn, "conditioning": cond, "resolution": res,
                             **score_cell(cell)})
    if not rows:
    ...
    fields = ["controlnet", "conditioning", "resolution"] + [
        k for k in rows[0] if k not in ("controlnet", "conditioning", "resolution")]
    ...
    for axis, label, fmt in [
        ...
        for res in resolutions:
            print(f"  res{res}")
            print(f"    {'':14}" + "".join(f"{c:>12}" for c in CONDITIONS))
            for cn in CONTROLNETS:
                line = f"    {cn:14}"
                for cond in CONDITIONS:
                    hit = [r for r in rows if r["controlnet"] == cn
                           and r["conditioning"] == cond and r["resolution"] == res]
    ...
    print("conditioning preprocessor.")
    print(f"\nsaved: {csv_path}")
    montage(resolutions)
