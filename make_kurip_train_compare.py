"""Build a kurip-source sample comparison montage."""
...
IMAGE_SIZE = 480
COMPARE_PATH = "results/compare_kurip_train_vs_shape1.png"
SAMPLES = [
...
def main():
    columns = ["rough", "shape1", "kurip_ft", "line (GT)"]
    header_h = 28
    label_h = 22
    canvas = Image.new(
        "RGB",
    ...
    for row, name in enumerate(SAMPLES):
        ...
        images = [
            ...
            load_image(f"results/shape1_kurip_samples/{name}_out.png"),
            load_image(f"results/kurip_samples/{name}_out.png"),
